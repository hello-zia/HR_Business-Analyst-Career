...
# Conduct data exploration to identify patterns and relationships across the dataset.

# Checking the Size of data 
length(df) #15
nrow(df) #4092

# Check for missing values: sum() to check NAs 
sum(is.na(df)) #10584
...
# Display the following columns: a. Job Title; b. Rating; c. Location; d. Industry
...
# a. Select the "Industry" column and remove duplicates
...
# In the context of ggplot2, hjust stands for "horizontal justification," and it is used to control 
# the horizontal alignment of text or labels on a plot. hjust takes a numeric value that represents 
# the horizontal justification, where:
# hjust = 0 aligns the text to the left.
# hjust = 0.5 aligns the text to the center.
# hjust = 1 aligns the text to the right.

# Save the plot to a file
ggsave("top_10_consulting_companies_plot.png", width = 10, height = 6) 
...
# SUMMARY 6: Top 5 Industries with the highest average rating
library(dplyr)
...
# SUMMARY 7: Top 5 Sectors with the highest average rating 
top_5_sectors <-df %>%
  group_by(Sector) %>%
  summarize(Average_Rating = mean(Rating, na.rm = TRUE)) %>%
  arrange(desc(Average_Rating)) %>%
...
# SUMMARY 8: Most common industry per ownership
library(dplyr)
...
# SUMMARY 9: Most common sector per ownership
...
# Linear Regression 2: Founded vs. Size 
...
# Linear Regression 3: Founded vs. Rating
...
# Multiple Regression: Founded, Rating, Size, and Revenue
